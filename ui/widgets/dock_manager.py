...
GUI-013 (2026-07-23): same treatment applied to YouTube Metadata
and Thumbnail, per Gary's request for the YouTube-related panels
to live in one tabbed slot instead of splitting the bottom dock
area into ever-narrower slivers alongside Transcript.
"""
...
class DockManager:
    ...
    def build(self, workspace):
        ...
        self._add(
            "Thumbnail", self.thumbnail_panel, Qt.BottomDockWidgetArea
        )
        self._add(
        ...
        self.main_window.thumbnail_panel = self.thumbnail_panel
        ...
        # GUI-013 (2026-07-23, per Gary's request): same tabify
        # treatment for the YouTube-related panels - YouTube
        # Metadata and Thumbnail were splitting the bottom dock area
        # three ways alongside Transcript, each getting a cramped
        # sliver of space. Tabbed together instead, each gets the
        # full bottom-area height when its tab is active. Transcript
        # stays a separate dock (not a "YouTube function" as such).
        self.main_window.tabifyDockWidget(
            self._docks["YouTube Metadata"], self._docks["Thumbnail"]
        )
        self._docks["YouTube Metadata"].raise_()
