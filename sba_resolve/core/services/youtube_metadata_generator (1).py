    ...
    def _build_prompt(self, ride_summary: dict, project_name: str) -> str:
        ...
        for day in ride_summary.get("days", []):
            ...
            if day.get("places"):
                ...
                parts.append(
                    "places: NONE IDENTIFIED - do not name any "
                    "country, region, or landmark for this day"
                )
...
CRITICAL - do not invent facts. This is the single most important
rule, more important than the brand voice guidance above:
- Do not name a country, region, city, or landmark unless it
  appears in the "places" field above. If a day says "NONE
  IDENTIFIED", write about that day without naming any location
  at all - do not guess a country or region from the camera
  brands, project name, or anything else.
- Do not name a specific motorcycle make or model. None was
  given to you, so do not invent one.
...
- Do not name specific historical sites, landmarks, roads, or
  route names unless they appear explicitly above.
